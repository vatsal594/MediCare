...
const MyAppointments = () => {
    ...
    const generateVideoCall = async (appointmentId) => {
        ...
        if (videoCallLinks[appointmentId]) {
            console.log("Video call link already exists:", videoCallLinks[appointmentId]);
            return;
        }
        ...
        try {
            ...
            if (data.success && data.callLink) {  // ✅ Checking success flag
                setVideoCallLinks((prev) => ({ ...prev, [appointmentId]: data.callLink }));
                        ...
                        <div className='flex flex-col gap-2 justify-end text-sm text-center'>
                            ...
                            {!item.cancelled && item.payment && !item.isCompleted && (
                                videoCallLinks[item._id] ? (
                                    <a href={videoCallLinks[item._id]} target="_blank" rel="noopener noreferrer" className='text-blue-600'>Join Video Call</a>
                                ) : (
                                    <button onClick={() => generateVideoCall(item._id)} className='text-blue-500'>Start Video Call</button>
                                )
                            )}
                        </div>
                    </div>
